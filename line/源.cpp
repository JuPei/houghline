...
double min_theta, double max_theta, Point start, Point end);//霍夫标准变换，输出line的rho和theta
static void
icvHoughLinesProbabilistic(CvMat* image, Mat& img,
float rho, float theta, int threshold,
int lineLength, int lineGap,
int linesMax, CvPoint start, CvPoint end, vector<vector<Vec4i>> result_lines);//概率霍夫变换，输出line的两个端点
struct  LinePolar
{
	float rho;
	float angle;
